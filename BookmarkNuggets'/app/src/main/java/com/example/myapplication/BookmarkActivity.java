...
public class BookmarkActivity extends AppCompatActivity {
    ...
    */
    ...
    Intent _intent;
    Gson _gson;
    ArrayList<LocationItem> _bookmark;
    Queue<LocationItem> _bookmarkData;
    int[] _IDs;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        _bookmarkData = getData("Bookmarks");
        _bookmark = new ArrayList<LocationItem>();
//        temp commented to do later
//        Upon creating, display the data for each view
        if(_bookmarkData == null){
            //do nothing because there's nothing in bookmark recorded
            System.out.println("NO BOOKMARK");
        }
        else{
            int _sizeOfBookmark = _bookmarkData.size();
            for(int i = 0; i < _sizeOfBookmark; i++){
                _view = (TextView) findViewById(_IDs[i]);
                _bookmark.add(_bookmarkData.poll());
                _view.setText(_bookmark.get(i).getTitle());
            }
        }
    ...
    public void viewMore(View view) {
        ...
        for (int i = 0; i < _bookmark.size(); i++) {
            LocationItem placeInfo = _bookmark.get(i);
